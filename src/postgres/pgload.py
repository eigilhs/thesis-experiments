...
def attribute_dict(line, *, attributes=re.compile(r'([^ ]+)="([^"]+)"')):
    return dict(attributes.findall(line))


def process_f40(file):
...
def process_f24(file, event_file, qualifier_file):
    ...
    for event in game:
        ...
        for qualifier in event:
            qj[qualifier.get('qualifier_id')] = parse_stuff(qualifier.get('value', True))
        
        event_file.writerow({'match_id': game.get('id'),
                             'qualifiers': json.dumps(qj),
                             **event.attrib})
        # for qualifier in event:
        #     qualifier_file.writerow({'event_id': event.get('id'), **qualifier.attrib})


#
# INSERT F40 SQUADS, COMPETITIONS, SEASONS, ...
#

with open('optaload/f40_tables.sql') as file:
...
for fname in glob('optadata/srml-*-squads.xml'):
    ...
    with open(fname) as file:
        competition, season, squad_entries, players, teams = process_f40(file)
    print('Inserting', fname, flush=True)
...
#
# INSERT F9 MATCH RESULTS
#

with open('optaload/f9_tables.sql') as file:
    cur.execute(file.read())
...
#
# INSERT F24 EVENTS AND QUALIFIERS
#

with open('optaload/f24_tables.sql') as file:
    cur.execute(file.read())
